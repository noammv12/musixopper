...
/// %LOCALAPPDATA%\Palon\whisper-runtime\, and are loaded manually in
        ...
        lock (Gate)
        {
            ...
            try
            {
                var dir = Path.Combine(
                    ...
                    "Palon", "whisper-runtime", Version);
                Directory.CreateDirectory(dir);

                var assembly = typeof(WhisperRuntime).Assembly;
                foreach (var name in LoadOrder)
                    ExtractIfNeeded(assembly, "Palon.WhisperNative." + name, Path.Combine(dir, name));
                foreach (var name in LoadOrder)
